...
function fieldCheckSummary(fieldCheck) {
  if (!fieldCheck) return null;
  return {
    mode: fieldCheck.mode,
    checkedAt: fieldCheck.checkedAt,
    account: fieldCheck.account ?? null,
    liveFields: (fieldCheck.fields ?? [])
      .filter((field) => field.source === "live-api")
      .map((field) => ({ id: field.id, label: field.label, evidence: field.evidence })),
...
export function matchStrategyToPlatforms(strategy, platforms, options = {}) {
  ...
  const platformMatches = platforms.map((platform) => {
    ...
    return {
      ...
      fieldCheck: fieldCheckSummary(liveFieldCheck),
      caveats
    };
  });

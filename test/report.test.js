...
test("parses Markdown campaign briefs into strategy input", async () => {
  ...
  assert.equal(strategy.product, "B2B workflow automation platform");
  assert.equal(strategy.locale, "GB");
  assert.ok(strategy.jobTitles.includes("Revenue Operations Manager"));
});
...
test("field checks fall back to registry fields when credentials are missing", async () => {
  ...
  const result = await checkPlatformFields(googleAds, { env: {} });
  assert.ok(result.fields.length > 0);
  assert.ok(result.errors.some((error) => error.code === "MISSING_AUTH" && error.recoverable));
});

test("reports include explicit live field-check evidence when supplied", async () => {
  const platforms = await loadPlatforms();
  const strategy = parseMarkdownBrief("Product: Test product\nMarket: B2B SaaS\nJob titles: VP Marketing");
  const output = matchStrategyToPlatforms(strategy, platforms, {
    fieldChecks: new Map([
      ["linkedin-ads", {
        ...
        fields: [
          { id: "jobTitle", label: "Job title", source: "live-api", evidence: "Test evidence." }
        ],
        errors: []
      }]
    ])
  ...
  assert.ok(report.includes("Read-only live API field check run at 2026-07-04T12:00:00.000Z"));
  assert.ok(report.includes("account/API-confirmed fields: Job title"));
});

...
test("maps generic B2B SaaS strategy to high-fit B2B platforms", async () => {
...
test("ranks exact B2B targeting above keyword-heavy proxies for generic B2B SaaS campaigns", async () => {
  const platforms = await loadPlatforms();
  const output = matchStrategyToPlatforms(
    {
      product: "B2B workflow automation platform",
      ...
      preferredChannels: ["linkedin-ads", "google-ads-youtube", "microsoft-ads"]
    },
    platforms
  );

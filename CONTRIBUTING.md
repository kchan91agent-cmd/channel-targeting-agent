...
Connector tests should use mocked `fetch` calls. Do not require live ad-platform credentials in `npm test`.

Only mark a field as `live-api` when an official authenticated endpoint confirms the mapped field or facet. If a connector only verifies account access, keep field-level claims registry-backed.
